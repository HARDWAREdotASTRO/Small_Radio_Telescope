...
void loop() {
  ...
  translateString(order, count_till, sped);
  pickFunction();
}  
...
int translateString(int &order, int &count_till, int &sped){
  //Convert from ASCII to decimal if there is new data availible.
  if (newData == true){
     order = (receivedChars[0] & 0xf);               //Direction of motor.
...
int Move(int &count, int &forLimitReach, int &revLimitReach){                   //Primary function for motor use.
...
int pickFunction() {
  if (order == 0){        //Do nothing loop. More of a precaution
    ...
    forLimitReach = 0;
    revLimitReach = 0;
    Move(count, forLimitReach, revLimitReach);
  }
